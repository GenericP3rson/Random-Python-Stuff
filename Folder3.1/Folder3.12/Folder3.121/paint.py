...
class Everything: 
    ...
    def labelled(self, dir='data', names=['Van Gogh', "Picasso"]):
        ...
        for pa in os.listdir(dir):
            ...
            labels.append(names[(int(pa.split('.')[0]))])
        return pix, np.array(labels)
    def buildModel(self, labelNum):
        m = k.models.Sequential()
        m.add(k.layers.Conv2D(10, kernel_size=(2, 2), activation = 'relu', input_shape=(100, 100, 3)))
        ...
        m.add(k.layers.Dense(labelNum, activation = "sigmoid"))
        m.compile(loss=k.losses.categorical_crossentropy,
                  optimizer=k.optimizers.Adam(), metrics=['accuracy'])
        return m
    def initLabels(self, names, labels):
        di = {}
        for i in range(len(names)): di[names[i]] = i
