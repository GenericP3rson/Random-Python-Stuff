...
#         # Makes sure the numbers don' out of controlt get
...
totX = []
totY = []
for p in range(seqLen, dataSZ, seqLen+1):
    inp = [toNum[i] for i in data[p-seqLen:p]]
    tar = [toNum[i] for i in data[p-seqLen+1:p+1]]
    totX.append(np.array(inp))
    totY.append(np.array(tar))
print(len(totX[0]))
totX = np.array(totX).reshape(len(data)//seqLen+1, seqLen,)
totY = np.array(totY).reshape(len(data)//seqLen+1, seqLen,)

m = k.Sequential()
m.add(k.layers.LSTM(100, input_shape=(seqLen-1, ), activation='tanh'))
m.add(k.layers.Dropout(0.1))
m.add(k.layers.LSTM(100, activation = 'tanh'))
m.add(k.layers.Dropout(0.1))
m.add(k.layers.BatchNormalization())
m.add(k.layers.Dense(100, activation = 'relu'))
m.add(k.layers.Dropout(0.1))
m.add(k.layers.Dense(seqLen-1, activation = 'sigmoid'))

m.compile(optimizer=k.optimizers.Adam, loss=k.losses.sparse_categorical_crossentropy)
m.fit(x = inp, y = tar, batch_size = 25, epochs=25, verbose=1)
